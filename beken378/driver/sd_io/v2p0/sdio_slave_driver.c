...
static bk_err_t sdio_slave_hw_init(void)
{
	...
	//clock
	/* TODO:sys_hal_set_sdio_clk_en(true);*/
	param = PCLK_POSI_SDIO;
	icu_ctrl(CMD_CONF_PCLK_26M, &param); /* sys_hal_set_sdio_clk_sel((uint32_t)SDIO_CLK_XTL);*/

	param = ICU_SDIO_CLK_DIV_1;
	icu_ctrl(CMD_SDIO_CLK_DIV, &param); /* sys_hal_set_sdio_clk_div((uint32_t)SDIO_CLK_DIV_1);*/
	...
	intc_service_register(IRQ_SDIO, PRI_IRQ_SDIO, sdio_slave_isr);/* bk_int_isr_register(INT_SRC_SDIO, sdio_slave_isr, NULL);*/
...
static bk_err_t sdio_hw_deinit(void)
{
	...
	param = ICU_SDIO_CLK_DIV_1;
	icu_ctrl(CMD_SDIO_CLK_DIV, &param); 
