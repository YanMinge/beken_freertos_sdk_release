...
enum hrp_meas_flags_bf
{
    ...
    HRS_FLAG_HR_VALUE_FORMAT_BIT                = COMMON_BIT(HRS_FLAG_HR_VALUE_FORMAT_POS),

    /// Sensor Contact Status bits
    /// Sensor Contact feature supported in the current connection
    /// Contact is detected
    HRS_FLAG_SENSOR_CCT_DETECTED_POS            = 1,
    HRS_FLAG_SENSOR_CCT_DETECTED_BIT            = COMMON_BIT(HRS_FLAG_SENSOR_CCT_DETECTED_POS),

    HRS_FLAG_SENSOR_CCT_FET_SUPPORTED_POS       = 2,
    HRS_FLAG_SENSOR_CCT_FET_SUPPORTED_BIT       = COMMON_BIT(HRS_FLAG_SENSOR_CCT_FET_SUPPORTED_POS),

    /// Energy Expended Status bit
    /// Energy Expended field is present. Units: kilo Joules
    HRS_FLAG_ENERGY_EXPENDED_PRESENT_POS        = 3,
    HRS_FLAG_ENERGY_EXPENDED_PRESENT_BIT        = COMMON_BIT(HRS_FLAG_ENERGY_EXPENDED_PRESENT_POS),

    /// RR-Interval bit
    /// One or more RR-Interval values are present. Units: 1/1024 seconds
    HRS_FLAG_RR_INTERVAL_PRESENT_POS            = 4,
    HRS_FLAG_RR_INTERVAL_PRESENT_BIT            = COMMON_BIT(HRS_FLAG_RR_INTERVAL_PRESENT_POS),
};
