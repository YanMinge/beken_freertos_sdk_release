...
enum wsc_feat_bf
{
    ...
    WSC_FEAT_TIME_STAMP_SUPP_BIT                = COMMON_BIT(WSC_FEAT_TIME_STAMP_SUPP_POS),

    /// Multiple Users Supported
    WSC_FEAT_MULTIPLE_USERS_SUPP_POS            = 1,
    WSC_FEAT_MULTIPLE_USERS_SUPP_BIT            = COMMON_BIT(WSC_FEAT_MULTIPLE_USERS_SUPP_POS),

    /// BMI Supported
    WSC_FEAT_BMI_SUPP_POS                       = 2,
    WSC_FEAT_BMI_SUPP_BIT                       = COMMON_BIT(WSC_FEAT_BMI_SUPP_POS),
...
enum wsc_meas_flags_bf
{
    ...
    WSC_MEAS_FLAGS_UNITS_IMPERIAL_BIT               = COMMON_BIT(WSC_MEAS_FLAGS_UNITS_IMPERIAL_POS),

    /// Time stamp present (bit 1)
    /// 0 for not present
    /// 1 for present
    WSC_MEAS_FLAGS_TIMESTAMP_PRESENT_POS            = 1,
    WSC_MEAS_FLAGS_TIMESTAMP_PRESENT_BIT            = COMMON_BIT(WSC_MEAS_FLAGS_TIMESTAMP_PRESENT_POS),

    /// User ID present (bit 2)
    /// 0 for not present
    /// 1 for present
    WSC_MEAS_FLAGS_USER_ID_PRESENT_POS              = 2,
    WSC_MEAS_FLAGS_USER_ID_PRESENT_BIT              = COMMON_BIT(WSC_MEAS_FLAGS_USER_ID_PRESENT_POS),

    /// BMI and Height present (bit 3)
    /// 0 for not present
    /// 1 for present
    WSC_MEAS_FLAGS_BMI_PRESENT_POS                  = 3,
    WSC_MEAS_FLAGS_BMI_PRESENT_BIT                  = COMMON_BIT(WSC_MEAS_FLAGS_BMI_PRESENT_POS),
};
